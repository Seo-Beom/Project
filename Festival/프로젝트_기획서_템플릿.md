...
graph LR
    ...
    subgraph STATE["State Management"]
        ZUSTAND2["Zustand Store<br/>- authStore<br/>- festivalStore<br/>- uiStore"]
    end
...
### 계층 구조 (Festival 도메인 예시)

com.ieum.app.backend.festival/
├── adapter/
│   ├── in/web/                       ← [Driving Adapter] 입구. @RestController 및 Request/Response DTO
│   └── out/persistence/              ← [Driven Adapter] 출구. Port 구현체 (EntityManager, Repository 사용)
│       ├── entity/                   ← JPA @Entity 클래스 (DB 매핑 전용)
│       └── repository/               ← Spring Data JPA Repository 인터페이스
├── application/
│   ├── port/
│   │   ├── in/                       ← [Input Port] UseCase 인터페이스 및 Command 객체
│   │   └── out/                      ← [Output Port] 상위 계층이 하위 계층에 요청하는 인터페이스 (Load/Save Port)
│   ├── result/                       ← [Application Result] 서비스가 반환하는 데이터 포맷 (예: FestivalListResult)
│   └── service/                      ← [Application Service] 쪼개진 단위 비즈니스 로직 구현체
└── domain/
    └── model/                        ← [Domain Model] 핵심 비즈니스 규칙을 담은 순수 객체
```

```
### 전체 패키지 구조 — user / admin 분리

📁 com.ieum.app.backend/
│
├── 📁 user/                                 # 👤 일반 사용자 서비스 영역
│   │
│   ├── 📁 festival/                         # 🎪 축제 도메인
│   │   ├── 📁 adapter/
│   │   │   ├── 📁 in/web/                   ← [Driving Adapter] 입구
│   │   │   │   ├── FestivalController.java  ← @RestController
│   │   │   │   ├── FestivalRequest.java     ← Request DTO
│   │   │   │   └── FestivalResponse.java    ← Response DTO
│   │   │   └── 📁 out/persistence/          ← [Driven Adapter] 출구. Port 구현체
│   │   │       ├── 📁 entity/
│   │   │       │   └── FestivalJpaEntity.java   ← JPA @Entity (DB 매핑 전용)
│   │   │       ├── 📁 repository/
│   │   │       │   └── FestivalJpaRepository.java ← Spring Data JPA Repository
│   │   │       └── FestivalPersistenceAdapter.java ← implements LoadFestivalPort, SaveFestivalPort
│   │   ├── 📁 application/
│   │   │   ├── 📁 port/
│   │   │   │   ├── 📁 in/                   ← [Input Port] UseCase 인터페이스 및 Command
│   │   │   │   │   ├── GetFestivalUseCase.java
│   │   │   │   │   ├── SearchFestivalUseCase.java
│   │   │   │   │   └── FestivalCommand.java     ← Command 객체
│   │   │   │   └── 📁 out/                  ← [Output Port] Load/Save Port
...
│   │   │       ├── GetFestivalService.java      ← implements GetFestivalUseCase
│   │   │       └── SearchFestivalService.java   ← implements SearchFestivalUseCase
│   │   └── 📁 domain/
│   │       └── 📁 model/                    ← [Domain Model] 핵심 비즈니스 규칙
│   │           ├── Festival.java
...
│   │   │   ├── 📁 port/
│   │   │   │   ├── 📁 in/
│   │   │   │   │   ├── SyncFestivalUseCase.java     ← 공공 API 수동 갱신
│   │   │   │   │   └── ManageFestivalUseCase.java
│   │   │   │   └── 📁 out/
│   │   │   │       └── PublicDataPort.java          ← 공공데이터 API 포트
│   │   │   ├── 📁 result/
│   │   │   │   └── SyncResult.java
│   │   │   └── 📁 service/
│   │   │       └── SyncFestivalService.java
│   │   └── 📁 domain/model/
│   │
│   ├── 📁 report/                           # 🚨 신고 관리
│   │   ├── 📁 adapter/ (in/web/, out/persistence/)
...
│       └── 📁 domain/model/
│
└── 📁 common/                               # 🔷 공통 인프라 (user/admin 모두 공유)
    ├── 📁 config/
    │   ├── SecurityConfig.java
    │   ├── CorsConfig.java
    │   └── SwaggerConfig.java
...
```
                    1주차            2주차            3주차            4주차
                 (3/25 ~ 3/31)   (4/1 ~ 4/7)     (4/8 ~ 4/14)    (4/15 ~ 4/24)
                ┌──────────────┬──────────────┬──────────────┬──────────────┐
 기획/설계       │██████████████│              │              │              │
 데이터(ERD/DB)  │██████████████│              │              │              │
 백엔드 API     │              │██████████████│██████████████│              │
...
| **4단계: 배포 & 마무리** | 4/15 ~ 4/24 | Docker, CI/CD, 통합 테스트, 버그 수정, Swagger 문서, 발표 PPT |
...
| 4주차 | 4/15 ~ 4/24 | ✅ Docker 컨테이너화, CI/CD 파이프라인, 통합 테스트, 크로스 브라우저 테스트, 버그 수정, Swagger 문서 정리, 발표 PPT, 시연 준비 | 전원 |
